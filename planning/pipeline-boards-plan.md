...
5. **Connectors** — ✅ shipped for crypto 2026-07-08 (the domain proven first, instead of stocks). Registry + provider input surface + bound fields + connector identity landed with the CoinGecko connector; then split into **connector (domain) → provider (backend)** with CoinMarketCap as a second backend + admin Connectors tab (`slice-5-coingecko-plan.md`, `slice-5b-crypto-provider-plan.md`). **5c (`slice-5c-connector-runtime-plan.md`)** extracted the agnostic **`runtime.js`** (adding a domain = one directory, zero runtime edits) and shipped **per-field liveness**: `mapping.fields[].live/every` = how often a field updates in the app, a worker `refreshDue` sweep over `entities.refresh_at`, `field_snapshots` history, and an **opt-in `retag_on_refresh`** board setting (tagging stays decoupled by default — the "dirty cascade" is off unless asked). Provider re-resolution by symbol landed here too. Commits c5c8981/67658a4/d512774/e678bed, pushed; 136 tests green; live-verified (a 1-min-live crypto board refreshes each minute, non-live fields stay frozen). **Stocks is the next connector — a sibling domain (one directory, `stocks_*` settings for free), not a template; zero stock code in core.** Still deferred: charts over `field_snapshots`, per-board provider override, per-provider rate-limit/backoff, liveness on AI fields.
