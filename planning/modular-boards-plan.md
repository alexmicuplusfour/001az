...
- **Web research (2026-07-06)** — the research capability shipped ahead of the stock type, as a **per-board toggle** (`boards.ai_research`, default off) rather than the adapter-manifest declaration planned below — a board can turn research off to save spend regardless of type, and the toggle let the machinery be proven on an image board before any stock code exists. Anthropic-only: `providers.js#anthropicRequest` (exported pure for tests) adds the `web_search` server tool (`max_uses: 5`), relaxes tool choice to auto, handles `pause_turn` continuations, and extraction name-checks `record_tags` (missing → retryable). Compat providers ignore the flag — OpenAI's web search is Responses-API-only and Gemini's compat layer has no grounding — so the adapter dossier stays the provider-neutral fallback, exactly as designed. Searches are counted (`ai_board_usage.search_count`, admin tooltips). Verified live: the tagger searched, identified a public figure from context clues, and cited it in the facet reasoning — and declined to search when the subject wasn't identifiable. When the stock type ships, its board-creation defaults should pre-set the toggle on.
- **Migration step 4 (2026-07-05)** — type plumbing complete. `GET /api/admin/board-types` serves the registry (`registry.list()`: type, name, `suggestedFacets` from the adapter); the board modal gains a **Board type** picker (create mode: choosing a type prefills its suggested facets into the facet editor, replacing an edited set only after a confirm; edit mode: disabled select — type is permanent). `POST /api/admin/boards` validates the type against the registry; PATCH never accepts it. The old hardcoded `defaultFacetsJson()` starter moved into the image adapter as its `suggestedFacets` (now with descriptions). Fixed in passing: public `GET /api/boards/:id` now returns `type` — the client's adapter resolution was silently riding the image fallback. Verified headless: picker populated, prefill + confirm flow, create, immutability in edit.

Not started: the stock type (steps 5–6) and the refresh loop (step 7). Sections below are updated where a shipped piece changed the design (marked ✅). **The refactor phase (steps 1–4) is complete.**

> **2026-07-06 — steps 5–7 superseded.** The board-type direction itself was retired: `agnostic-core-plan.md` dissolves the image module into an agnostic core (items = identity + files + fields, no type registry), and `pipeline-boards-plan.md` builds the comparator features on top (universal input, identity modes, extraction fields, connectors); "stocks" ships as a template + connector, never an adapter. This document is historical record of the shipped refactor (steps 1–4).
...
4. ✅ **Type plumbing (2026-07-05):** board-types API + type picker in the board modal (permanent after creation), suggested-facet prefill with confirm-on-overwrite, type validation on create, `type` in the public board payload.
