...
- Dev loop: `docker compose up -d --build app` → http://localhost:8001; `npm test` (needs compose pg on 127.0.0.1:5433); mint a session: `docker compose exec app node server/mintlink.js alexmicuplusfour@gmail.com` then curl the link with a cookie jar. Headless checks: Edge + `npm i --no-save puppeteer-core` (prune after). Live verify on throwaway boards via the API, delete them after. Real tagging calls are fine (default Anthropic key configured).
