...
### 2. [MED — FIXED 2026-08-01] Ghost empty entities are now reachable — and nothing reaps them

Under the old FK + single-transaction `reparentInstance`, a zero-instance entity was structurally impossible. `listItems` (db.js:210-250) maps **every** entity row to a card, so a zero-instance entity renders as a blank ghost card (`instances:[]`, no face → tile), and nothing sweeps them (`deleteEntityIfEmpty` only runs from `reconcileEntities` and the instance-delete heal). Closer look split this into three, not two:
- **(a) Non-atomic membership write** *(the real regression)* — `setItemEntities` then `reconcileEntities` are two separate auto-commits in `extractOne`. The old `reparentInstance` was one `withTx` *specifically* so a crash couldn't strand a ghost. A crash between them leaves the abandoned sole-instance entity empty AND unreaped — and re-extraction can't heal it (the item no longer references it, so reconcile never revisits it). **Permanent** blank card.
- **(b) Concurrent instance-delete race** *(over-weighted in the first pass — walked back)* — server.js:2306-2320: two concurrent last-two deletes both pass the `entityInstanceCount ≤ 1` guard, but the `deleteEntityIfEmpty` heal DOES close it (whichever heal runs after both DELETEs commit sees the empty entity; the emptiness check is atomic). Residue is only a crash between the delete and the heal — **narrow, LOW**.
- **(c) FK-loss dangling id** *(distinct race)* — `entity_ids` has no FK, so an extraction resolving to entity `E` can append `E` to an item *after* `deleteEntity` scrubbed and dropped it → a dead id (cosmetic if the item has other entities; a lost instance if `E` was its only one). The `deleteEntity` header comment still described the OLD FK-based protection.

*Fixed (2026-08-01):* (a) the extract pair is wrapped in `withTx` (worker.js) — membership write + reconcile commit or roll back together, restoring `reparentInstance`'s atomicity. Added `reapEmptyEntities(db, olderThanMs)` (db.js) + an hourly `reapGhostEntities()` tick in the maintenance loop (`ENTITY_REAP_AFTER_MS`, default 30 min) as the backstop the commit anticipated — with a load-bearing **age floor** so it never reaps an in-flight upload's entity (created one statement before its instance). Stale `deleteEntity` comment corrected to describe the explicit orphan cleanup + the no-FK (c) gap the reaper backstops. Tests: withTx roll-back-together + four reaper cases (drops settled ghost / spares fresh / spares populated even when aged); age-floor mutation-verified. Suite 603/603. (c)'s full prevention (re-lock in `setItemEntities`, or the reaper also scrubbing dead ids) left as a follow-up — the reaper cleans the emptied-entity half.
