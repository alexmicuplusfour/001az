...
4. **Derived identity** — identity `from: "ai"` in the mapping; extraction derives the identity value instead of using the filename. Normalize (trim, case/whitespace collapse); collision on `(board_id, identity)` merges — files append, merged entity re-extracts, re-tags. File removal in Details as the unmerge path. *Verify: two docs for the same person → exactly one entity with both files.*
