...
2. **Extraction** — `boards.mapping` (AI fields only), mapping stamped on items, `record_fields`, `pending_extract`/`extracting`, fields in Details. **The plus splits here**: plus keeps today's file picker exactly; a chevron menu gains its first entry, "Entity mapping…" → the mapping modal (blank-first, no templates; field rows with a source select — "AI instruction" active, "Connector" visibly disabled as the forward signal). The mapping is never board-settings UI — the menu and modal grow in place across slices 3/5. Anthropic-first like tagging. Implementation plan: `slice-2-extraction-plan.md`. *Verify: hand-written fields on a doc board → extracted values with justifications in Details.*
3. **Ingestion modal + review** — split plus, template picker (board default, switchable per ingestion), `review` staging status, review grid (identity + fields + face), commit/discard, auto-commit toggle. *Verify: mis-described field caught at review; nothing lands unseen.*
4. **Derived identity** — identity `from: "ai"`, normalize/merge/accumulate, merge warnings in review, file removal in Details as the unmerge path. *Verify: mixed Watson/Roberts images → exactly two entities, merge visible at review.*
5. **Connectors** — registry, finnhub (settings, fns, `search`, chart `preview`, stock template), provider input surface, bound fields + identity, `gather_every_min` + dirty cascade, `field_snapshots`. **Stocks ship as a template, zero stock code in core.** *Verify: search "intel" → review shows Intel Corp + chart + exact numbers → commit → tagged; next cycle re-tags only changed entities.*
