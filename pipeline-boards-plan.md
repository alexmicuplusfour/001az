...
1. **Documents** — ✅ **shipped 2026-07-06.** `server/sources/doc.js` (pdf/txt/md/csv; pdf page-1 preview via poppler in the app image, graceful no-preview fallback; magic-byte + NUL-byte sniffing, 100-page cap) behind a `server/sources/index.js` dispatcher with the shared cleanup; `/api/items` + upload response gained additive `kind`/`label`; `modelInputFor` sends pdf document blocks / inline text (50k char cap); compat providers reject document parts with a clear "use an Anthropic tagger" error; client `docKind` (preview-or-badge + filename title, measured layout lane, detail = original in a new tab), lightbox strip filters to images on mixed boards. *Verified: 51 tests; live — PDF + txt on a candidates board tagged correctly by content (senior vs staff-plus, reasoning quotes the documents), poppler preview rendered, headless card faces clean, board delete removed all three stored files.*
2. **Text input** — plus dialog (files + lines), `{ lines }` ingest, `payload.text`, title-only cards, text part in model input. *Verify: paste ticker lines on a research-on board → tagged with sane reasoning.*
