// Object-detection slot (Slice 1): the fourth AI capability beside tag / embed /
// transcribe. Mirrors the transcriber tests — the descriptor advertises `detects`,
// the catalog exposes it, detectObjects dispatches through the provider's
// wire.detect, and resolveDetector resolves the on-device OWLv2 detector by
// default (a capability-gated provider override falls back to it). None of these
// run the real OWLv2 model — a stub wire stands in for the pipeline, so the suite
...
test("descriptor: the on-device detector advertises detects with a real wire.detect", () => {
  const d = PROVIDERS.localDetector;
  assert.equal(d.onDevice, true);
  assert.equal(d.keyless, true);
  assert.equal(typeof d.wire.detect, "function");
  assert.equal(d.detects.default, "Xenova/owlv2-base-patch16-ensemble");
});
...
test("catalog: detects rides through with default + models; a non-detect provider is null", () => {
  ...
  assert.equal(c.detects.default, "Xenova/owlv2-base-patch16-ensemble");
...
test("dispatch: detectObjects routes to wire.detect, passing image + queries + threshold", async () => {
  let seen = null;
  registerProvider("stubdet", {
    label: "StubDet",
    onDevice: true, // exempt from rate-limit contract, like the on-device detector
...
test("resolveDetector defaults to the on-device OWLv2 detector (no settings)", async () => {
  const d = await resolveDetector(db);
  assert.equal(d.id, "localDetector");
  assert.equal(d.model, "Xenova/owlv2-base-patch16-ensemble");
...
test("GET ai-config exposes the detect block; POST gates the provider and stores the threshold", async () => {
  ...
  assert.equal(cfg.detect.threshold, 0.1);
