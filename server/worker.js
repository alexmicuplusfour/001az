...
Also judge whether the image is the kind of material this board collects, as described above and implied by the facet vocabulary. Default to "match": if the image is broadly the right kind of material, it is a match even when it sits at the edges of the collection's scope or you are unsure which specific tags apply — uncertainty about tags is handled by tagging conservatively, not by "undecided". Set "fit" to "undecided" only when the image is plainly a different kind of material altogether (e.g. a photograph, artwork, or text document on a board that collects UI screenshots), so a human can review it.
