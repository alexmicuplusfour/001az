...
export default function ({
  ...
  series: {
    unsupported, createTtlCache, ytdDays, walkLadder, keyFingerprint,
    encodeArea, encodeCandles, CHART_TTL_LIVE, CHART_TTL_SETTLED,
  },
}) {
  ...
  const description = "Live crypto prices — needs a key";
  const needsKey = true;
  const attribution = { text: "Data by CoinMarketCap", url: "https://coinmarketcap.com" };
  ...
  // caches here, not by rpm. The burst is the runtime's default.
  ...
  async function fetchEntity(id, ctx = {}) {
    ...
    // 404, as CoinGecko's: an unknown id fails its item at once without
    // pausing the provider (plugin-contract-plan.md, Stage 5).
    if (!d) throw Object.assign(new Error(`CoinMarketCap: no data for id ${id}`), { status: 404 });
    return {
      id: String(d.id),
  ...
  async function history(id, period, { apiKey, pace } = {}) {
  ...
  const honorsSorts = Object.keys(SORT_FIELD);
  ...
  async function list({ sort, order, page = 1, pageSize = 50, query } = {}, { apiKey, pace } = {}) {
  ...
  return {
    label, description, needsKey, attribution, pacesRequests, rpm, burst, search,
    maxPageSize, prefetch, fetchEntity, fetchFields, history, chart,
    honorsSorts, list, testConnection,
    // test seams
    _resetQuoteCache, _resetChartCache,
  };
}
