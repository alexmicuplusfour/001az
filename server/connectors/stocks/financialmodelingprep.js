...
export default function ({
  providerSignal, providerBudgetMs, num,
  series: {
  ...
  const label = "Financial Modeling Prep";
  const description = "US stock quotes, fundamentals, and price history — needs a key";
  const needsKey = true;
  ...
  // constant answers.
  const attribution = { text: "Data by Financial Modeling Prep", url: "https://financialmodelingprep.com" };
  ...
  async function fetchEntity(id, { apiKey, pace } = {}) {
    ...
    // 404: an answer about this symbol, not about FMP — the item fails at once
    // and the provider isn't paused (plugin-contract-plan.md, Stage 5).
    if (!quote) throw fail(`no quote for ${requested}`, 404);
  ...
  async function fetchFields(id, keys, { apiKey, pace } = {}) {
  ...
  return {
    label, description, needsKey, attribution, pacesRequests, rpm, burst,
    search, fetchEntity, list, fetchFields, periods, history, chart,
    filterOptions, testConnection,
    // test seams
    _resetScreenerCache, _ageScreenerCache, _resetChartCache, _ageChartCache,
  };
}
