...
export function resolvePreset(id) {
  ...
  if (id != null && !p) console.warn(`unknown image preset "${id}" — using "${DEFAULT_PRESET}"`);
...
// ({ source, edge?, quality?, bytes, fallback? }) the job log records, so
// "what did the model actually see" stays answerable.
export async function aiImageFor({ galleryDir, thumbsDir }, file, { preset = IMAGE_PRESETS[DEFAULT_PRESET], images = GENERIC_IMAGES } = {}) {
  ...
  const readThumb = async (fallback) => {
    const buf = await fs.promises.readFile(thumbPath);
    return part(buf, { source: "thumb", bytes: buf.length, ...(fallback ? { fallback } : {}) });
  };
  ...
  try {
    ...
    if (!out) {
      console.warn(`ai image for ${file.name}: over ${maxBytes}B after quality/size step-down — sending the thumbnail`);
      return readThumb("byte-cap");
    }
    return part(out.data, { source: "original", edge: out.edge, quality: out.quality, bytes: out.data.length });
  } catch (err) {
    console.warn(`ai image for ${file.name}: ${err.message} — sending the thumbnail`);
    return readThumb("render-error");
  }
}
