...
// factory an ai-provider plugin exports — so a built-in and a dropped-in plugin
// enter the registry through the identical path; the only difference is a
// plugin is stamped `external`. The engine (providers.js) calls each with
// { wires } and registers the result. This file is the ONE place the built-in
// roster is named; adding a built-in is one import + one entry.
//
