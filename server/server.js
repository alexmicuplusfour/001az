...
import { startWorker, invalidateBoardCache, invalidateAllBoardCaches, resolveEmbedder, resolveTranscriber, servedBacklogLanes, engineStamp, nextAutoTagRun } from "./worker.js";
...
import { loadAll as loadPlugins, installFromUrl, updatePlugin, uninstall, pluginsDir } from "./plugin-loader.js";
...
// The operator's lock on running new code (PLUGIN_INSTALL_DISABLE=1, read per
// request): install and update refuse every source but the image's own
// examples, which are the app's code like the built-ins — so the welcome
// flow's bundled install still works. Installed plugins keep loading and
// Remove keeps working: the switch closes the door, it doesn't empty the room.
const installLocked = () => process.env.PLUGIN_INSTALL_DISABLE === "1";
const INSTALL_LOCKED = "installing plugins is turned off on this server (PLUGIN_INSTALL_DISABLE) — only the bundled examples can be added or updated";
...
app.patch("/api/admin/plugins/:id", requireAdmin, wrap(async (req, res) => {
  ...
  if (installed !== undefined) {
    ...
    // An external plugin is present or uninstalled, never switched off: its
    // card reads installed from the install record (pluginCatalog), while
    // resolution and the connector runtime read this row — `false` here would
    // make the two disagree. `true` stays accepted: it is the row install
    // already wrote, and the welcome flow sends it for the bundled plugin it
    // has just installed.
    if (!installed && (await getExternalPlugin(db, def.id)))
      return res.status(400).json({ error: "external plugins are removed, not toggled" });
...
app.post("/api/admin/plugins/install", requireAdmin, wrap(async (req, res) => {
  ...
  if (installLocked() && !isBundledSource(url)) return res.status(403).json({ error: INSTALL_LOCKED });
  try {
...
// Clusters by meaning (plan 3-meaning): the server carves, the client gets
// the carving — same deterministic k-means the chip flavor runs in the
// browser (cluster-core.js, imported by both sides so the seeding rules
// have one home), over the stored embeddings, wired as conclusions (tens of
// KB) rather than raw vectors (~2.8MB and growing). Free like the similar
// route: nothing embedded, nothing metered. Cached per (board, level)
// against a corpus fingerprint — recomputed at most once per data change,
// which is what absorbs the compute (~300ms at 1.8k items).
//
// Groups wear gibberish HANDLES hashed from their most typical member:
...
app.get("/api/boards/:id/meaning-clusters", requireAuth, wrap(async (req, res) => {
  ...
  const embedder = await resolveEmbedder(db);
  if (!embedder) return res.status(404).json({ error: "semantic search is not enabled" });
  const level = Math.min(Math.max(parseInt(req.query.level, 10) || 1, 1), CLUSTER_LEVEL_MAX);
  const { rows: [fpRow] } = await db.query(
    "SELECT COUNT(*)::int AS n, COALESCE(MAX(updated_at), 0)::text AS t FROM items WHERE board_id=$1 AND embedding IS NOT NULL AND embedding_model=$2",
    [boardId, embedder.model]);
  const fp = `${embedder.model}|${fpRow.n}|${fpRow.t}`;
  const cacheKey = `${boardId}|${level}`;
  ...
  if (corpus?.fp !== fp) {
    ...
    ids.forEach((id, i) => {
      ...
      let s = 0;
      for (let j = 0; j < dims; j++) s += sum[j] ** 2;
      s = Math.sqrt(s) || 1;
